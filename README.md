...
1 * Project contains an Android library with an activity that displays jokes passed to it as intent extras.
1 * Project contains a Google Cloud Endpoints module that supplies jokes from the Java library. Project loads jokes from GCE module via an async task.
0 * Project contains connected tests to verify that the async task is indeed loading jokes.
0 * Project contains paid/free flavors. The paid flavor has no ads, and no unnecessary dependencies.
...
https://github.com/GoogleCloudPlatform/gradle-appengine-templates/tree/master/HelloEndpoints
https://github.com/DmitryMalkovich/build-it-bigger-app/blob/master/app/src/main/java/com/dmitrymalkovich/android/builditbigger/RetrieveJokeTask.java